...
- **배포**: 2026-09-13 15:54 `deploy.sh` → 이미지 `hotdeal-bot:20260913_1554`, 커밋 `643bb17`. ArgoCD 자동 동기화로 약 75초 후 새 파드 `dev-hotdeal-bot-8558776b7-mljj2` Running, 구 파드(4/23 이미지, 142일 기동) 정상 종료. 기동 로그에서 "애플 리퍼비쉬 감시 태스크 시작 (10분 주기, 트리거 키워드 '애플리퍼')" 및 등록자 없음으로 크롤링 생략 확인
